"use client";
import { Toolbar } from "@/types/interface";
...
export default function Bill() {
    ...
    const { mutate: updateMutate } = useQueryApi(
        "bill-templates",
        "bill-templates",
        "PATCH",
    );
    const { mutate: deleteMutate } = useQueryApi(
        "bill-templates",
        "bill-templates",
        "DELETE",
    );
    const { mutate: generateMutate } = useQueryApi(
        "generate-bills",
        "bills",
        "POST",
    );
    ...
    const items = data?.map((item: any) => ({
        ...
        description:
            item.billName + " " + item.currFreq + " / " + item.frequency,
        amount: item.billAmount,
    }));

    if (isLoading) {
        return <CardListSkeleton row={5} />;
    }
    return (
        <>

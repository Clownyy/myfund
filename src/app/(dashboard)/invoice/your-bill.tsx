...
export default function YourBill() {
    ...
    const { mutate: disburseMutate } = useQueryApi(
        "disburse",
        "bills",
        "PATCH",
    );

    const items = data?.map((item: Bill) => ({
        ...item,
        transType: item.paid ? "Paid" : "Unpaid",
        variant: item.paid ? "success" : "destructive",
        description: item.notes,
        amount: item.template.billAmount,
    }));
